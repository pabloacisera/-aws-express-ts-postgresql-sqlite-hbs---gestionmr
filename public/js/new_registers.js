...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const checkboxes = [
    {
      check: "c_matriculacion_check",
      fields: ["c_matriculacion_venc", "c_matriculacion_cert"],
    },
    ...
    .addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const dataForSend = {
          ...
          c_matriculacion_venc: document.getElementById("c_matriculacion_check")
            .checked
            ? formData.get("c_matriculacion_venc")
            : null,
          c_matriculacion_cert: document.getElementById("c_matriculacion_check")
            .checked
            ? formData.get("c_matriculacion_cert")
            : null,

          seguro_venc: document.getElementById("seguro_check").checked
            ? formData.get("seguro_venc")
            : null,
          seguro_cert: document.getElementById("seguro_check").checked
            ? formData.get("seguro_cert")
            : null,

          rto_venc: document.getElementById("rto_check").checked
            ? formData.get("rto_venc")
            : null,
          rto_cert: document.getElementById("rto_check").checked
            ? formData.get("rto_cert")
            : null,

          tacografo_venc: document.getElementById("tacografo_check").checked
            ? formData.get("tacografo_venc")
            : null,
          tacografo_cert: document.getElementById("tacografo_check").checked
            ? formData.get("tacografo_cert")
            : null,
        };
  ...
  // Limpiar formulario
  document
    .getElementById("btn-clean-control")
