...
export class RegistersCacheService {
  ...
  private static stripCacheFields(obj: any): any {
    ...
    if (result.certificates && typeof result.certificates === "string") {
      try {
        ...
        console.error("Error parseando certificados JSON:", e);
        result.certificates = [];
      }
    } else if (!result.certificates) {
      result.certificates = [];
    }
  ...
  static async searchCacheRegisters(
    ...
    const countStmt = this.db.prepare(
      `SELECT COUNT(*) as total FROM ControlRegister ${whereClause}`,
    );
  ...
  static async createNewRegister(data: dataControl) {
    ...
    const licenciaVencimiento = data.licencia_vencimiento
      ? new Date(data.licencia_vencimiento).toISOString()
      : null;
    const cMatriculacionVenc =
      data.c_matriculacion_venc instanceof Date
        ? data.c_matriculacion_venc.toISOString()
        : data.c_matriculacion_venc;
    const seguroVenc =
      data.seguro_venc instanceof Date
        ? data.seguro_venc.toISOString()
        : data.seguro_venc;
    const rtoVenc =
      data.rto_venc instanceof Date
        ? data.rto_venc.toISOString()
        : data.rto_venc;
    const tacografoVenc =
      data.tacografo_venc instanceof Date
        ? data.tacografo_venc.toISOString()
        : data.tacografo_venc;
  ...
  static async deleteRegistry(controlId: number): Promise<boolean> {
    const stmt = this.db.prepare("DELETE FROM ControlRegister WHERE id = ?");
  ...
  `);
    ...
    const result = stmt.run(
      ...
      data.documentSummary ||
        '{"total":0,"types":[],"byType":{},"latestDocument":null}',
  ...
  static async createNewRegisterWithId(
    id: number,
    data: dataControl,
  ): Promise<boolean> {
    const stmt = this.db.prepare(`
  ...
  static async updateTempIdToRealId(
    tempId: number,
    realId: number,
  ): Promise<boolean> {
    const stmt = this.db.prepare(
      "UPDATE ControlRegister SET id = ?, _cached_at = datetime('now') WHERE rowid = ?",
    );
  ...
  static async syncFullRegistryFromPostgres(registry: any): Promise<void> {
    const registryData = {
      ...
      deletedAt:
        registry.deletedAt instanceof Date
          ? registry.deletedAt.toISOString()
          : registry.deletedAt,
    };

    const success = await this.updateRegistry(registry.id, registryData);
    if (!success) {
      await this.createNewRegisterWithId(
        registry.id,
        registryData as dataControl,
      );
    }
  }
    ...
    `,
      ...
      `);
        ...
        const columns = this.db
          .prepare(`PRAGMA table_info(ControlRegister)`)
          .all();
        ...
        if (!columnNames.includes("certificates")) {
          ...
          this.db
            .prepare(
              `ALTER TABLE ControlRegister ADD COLUMN certificates TEXT DEFAULT '[]'`,
            )
            .run();
        }
        ...
        if (!columnNames.includes("documentSummary")) {
          ...
          this.db
            .prepare(
              `ALTER TABLE ControlRegister ADD COLUMN documentSummary TEXT DEFAULT '{"total":0,"types":[],"byType":{},"latestDocument":null}'`,
            )
            .run();
        }
        ...
        if (!columnNames.includes("user")) {
          ...
          this.db
            .prepare(
              `ALTER TABLE ControlRegister ADD COLUMN user TEXT DEFAULT '{"id":0,"name":"","email":""}'`,
            )
            .run();
        }
